...
import matplotlib.pyplot as plt, itertools, numpy as np, pandas as pd
...
def read_all_measures(table_name, seed = 0):
    ...
    , then take all objects with all magnitudes aper measures
...
def read_shuffle_cut(table_name:str, newtable_size:int, seed = 0):
    ...
    Then pick objects randomly, given a random seed, to a new table with newtable_size
    ...
    def unique_random(table, ind_range, subset_size:int):
        ...
        subset_size : int
            number of objects to catch in table between minimum and maximum indices
        seed: int
        ...
        np.random.seed(seed)
        #choose indices
        rand_ind = np.random.choice(np.arange(ind_range[0], ind_range[1]), subset_size, replace = False)
        #assist table
        return table[rand_ind]
         
    
    def ntble(table, table_size:int, newtable_size:int, N:int, n:int, r = 0, seed = 0):
    ...
    if (s < len(table)) and s != 0:
        ...
        N = T // s
        #if number of small tables are smaller than size of every small table (subset)
        if N < s:
        ...
        else:#if number of small tables are greater than size of every small table (subset)
...
def gen_df_features_targets(data, splus = False, normal = True, targets = True):
    ...
    data : pandas.core.frame.DataFrame
        Pega as features do S-PLUS. Pode ser cor: splus = True, só as mags: 
    ...
    normal : TYPE, optional
        Se True, retoma um DataFrame com as diferenças de magnitude
        dos filtros largos com os filtros de bandas estreitas que operam na mesma
        banda daquele filtro largo. Por fim, há diferenças de magnitude em relação
        ao filtro r. The default is True.
    ...
    if normal == False and splus == False:
        normal = True
        
    if normal == True:
        
        header = ['f378 - ujava', 'f395 - g', 'f410- g','f430 - g', 'f515 - g',\
              'r - f660', 'f861 - z', 'ujava - r', 'g - r', 'r - i', 'r - z']
    ...
    if splus == True:
        ...
        header2 = ['ujava_aper', 'f378_aper', 'f395_aper', 'f410_aper', 'f430_aper',\
                   'g_aper', 'f515_aper', 'r_aper', 'f660_aper', 'i_aper', 'f861_aper'\
                       , 'z_aper']
        ...
        try: features_df = pd.concat([features2, features_df], axis = 1)
        except: features_df = features2        
...
def genft_df_gaia(data, null = True, positive_parallax_only = True,\
    ...
    """
    Retorna um DataFrame com as features do GAIA e pode retornar outro DataFrame
    com os índices dos objetos que se encaixam nas condições pedidas, Index = True.
    
    Pode pegar todos os objetos com paralaxe ou tomar só os com valores positivos,
    positive_parallax_only = True
    
    Pega também só objetos que tenham valores não nulos(nan) nas medidas pedidas,
    null = True
    
    Também pode dar só valores bp, rp, parallax, normal = False ou bp - rp, e MAG G
    normal = True
    ...
    if normal == False:
        ...
        #features
        features_df = pd.concat([features_df, ABS_MAG(data,['phot_g_mean_mag'],\
                                                      data['parallax'])], axis = 1)
    
        features_df.rename(columns = {'PHOT_G_MEAN_MAG' : 'MAG G'}, inplace = True)
        
        features_df['bp - rp'] = data['phot_bp_mean_mag'] - data['phot_rp_mean_mag']
    ...
    if null == True:
        ...
        if positive_parallax_only == True:
...
def concat(data1, data2, seed_concat = 0):
    """
    Adiciona(concatena) um DataFrame ao outro e embaralha os objetos com seed = 0
...
def gen_ft_dif_splus_x(data, dif = 'r'):
    """
    Retorna um Pandas DataFrame com as cores em relação ao filtro r do S-PLUS.
    Queria por para qualquer filtro, mas acho que tinha que por tantos ifs, que
    melhor deixxar assim
    ...
    """
    header_r = ['u - r', 'f378 - r', 'f395 - r', 'f410 - r', 'f430 - r', 'g - r',\
                'f515 - r', 'r - f660', 'r - i', 'r - f861', 'r - z']    
...
def ABS_MAG(DataFrame, header: list, parallax, new = True, bottom = False):
    """
    Retorna Dataframe com as magnitudes absolutas do DataFrame de entrada. Precisa
    especificar quais colunas vai querer transformar por meio do header.
    Pode ou não retornar o mesmo DataFrame acrescido das magnitudes absolutas com
    parâmetro new e colocar ou não no final do DataFrame com bottom.
    ...
    bottom : bool, optional
        ...
        Se bottom = False, retorna as magnitudes absolutas nas primeiras colunas.
        Se bottom = True, retorna as magnitudes absolutas nas últimas colunas.
        The default is False.
    ...
    if new == True:
        ...
        if bottom == False: MAGs = pd.concat([MAGs, DataFrame], axis = 1)
            
        else: MAGs = pd.concat([DataFrame, MAGs], axis = 1)
...
def MAG(mags, parallax):
    """
    Dá Pandas DataFrame com magnitudes absolutas das magnitudes aparentes e pa-
    -ralaxe fornecidas
...
def prize_random(features, targets, testsize = .2, prize = 100, \
                  title='Confusion matrix', plot = False, save = False, \
                      return_all = False):
    """
    Pega os objetos com features e seus respectivos targets. Divide em conjuntos
    de treinamentos e testes. O tamanho padrão é 80% de treinamentos e 20% de 
    testes, pode-se mudar o tamanho do conjutno de teste e treinamento será 
    1 - x%. Salva ou plota a Confusion Matrix
    Para cada treinamento/teste, é gerado uma seed que sorteará os conjuntos de
    treinamento/teste. O sorteio está vinculado a essa seed. Assim, se for comparar
    diferentes features, o sorteio será o mesmo para cada um.
    ...
    for train_seed in range(prize):
        ...
        train_fset_fgs, test_fset_fgs, train_tset_fgs, test_tset_fgs = train_test_split\
            (features, targets, test_size = testsize, train_size = trainsize,\
    ...
    model_cm = confusion_matrix(y_true = targ_set[j][1].iloc[:,0], y_pred = \
                                pred[j], labels=class_labels)
...
def rec_all(table1, table2, target_test, predicted, classe = 'WD'):
    ...
    classe : str, optional
        ...
        Com base nela que serão classificadas VP, FP, FN e VN no cabeçalho (header)
        CRF
    Returns
    -------
    Pandas DataFrame
    ...
    tfp_tfn = pd.concat([true_positive, false_positive, true_negative, false_negative], \
                    ignore_index = True)
    ...
    for a in M.columns:
        ...
        for (b, c), d in zip(enumerate(HEADER_X[:-1]), header_xlabel):
            ...
            for f,g in zip(HEADER_X[(b + 1):], header_xlabel[(b+1):]):
                ...
                rows = ['True Positive', 'False Negative', 'True Negative', 'False Positive']
                n_rows = len(rows)
                colors_l = ['tab:blue', 'tab:red', 'tab:green', 'tab:orange']
                ax.set_position([0.04, 0.025, 1.1, 0.9])#pos : [left, bottom, width, height]
                ...
                ax.plot(x1, y1, 'o', marker = 'o', markersize = mk + .1, \
                         alpha =.8, color = colors_l[0], zorder = 5, label = 'True Positive')
                
                        
                ax.plot(x2, y2, 's', markersize = mk + .2, alpha = .6, color = colors_l[1],\
                        zorder = 20, label = 'False Negative')
    
                for n, x2n in enumerate(x2):
                    ax.annotate(n + 1, (x2n, y2.iloc[n]), color = colors_l[1])
                    
                ax.plot(x3, y3, 'o', marker = '.', markersize = mk -.1, alpha = .9, color = \
                        colors_l[2], zorder = 0, label = 'True Negative')
                
                ax.plot(x4, y4, 'x', markersize = mk + .4, alpha = .7, color = colors_l[3], \
                        zorder = 10, label = 'False Positive')
                for n, x4n in enumerate(x4):
                    ax.annotate(n + 1, (x4n, y4.iloc[n]), color = colors_l[3])
                table = ax.table(cellText = cell_text, colLabels = col_label, rowLabels = \
                                 rows, rowColours = colors_l, in_layout = True,\
                                 bbox = (1.0835,0,.025,.4))#bbox parameters (xi, yi, width, height)
                ax.invert_yaxis()
                table.scale(1,4)
...
def sdss_spec(DataFrame, Title, save = True, \
              ...
              allset = True):
    """
    Baixa os espectros de objetos de um DataFrame. Os espectros são do SDSS e 
    deve ter no cabeçalho do DataFrame: Plate, MJD e Fiber. Os espectros são 
    salvos no local_save.
    Pode-se optar por não salvá-los na máquina. Nesse caso deve-se mexer nas con-
    -figurações padrões do fetch_sdss_spectrum. 
    ...
    figure_size = (21,10)
    f_dpi = 300
    if save == True: pdfsp = PdfPages(Title + '.pdf')
    ...
    for i,j in enumerate(lista):
        ...
        for k,row in enumerate(j.itertuples()):
            ...
            try:
                spec = fetch_sdss_spectrum(plate, mjd, fiber, data_home = local_save)
                if save == True:
                    fig, ax = plt.subplots(num = figure_number, figsize = figure_size,\
                                                           dpi = f_dpi, clear = True)
                    ...
                    ax.set_title(Title + ' ' + row.CRF + ' ' + str(k + 1) + ' ' 'Plate = %(plate)i, MJD = %(mjd)i, Fiber = %(fiber)i' % locals(),\
                                 color = colors_l[i])
                    ...
                    ax.set_ylabel('$Flux \hspace{.6} (10^{-17} ergs \hspace{.6} cm^{-2} \hspace{.6} s^{-1 } \hspace{.6} \AA)$')
                    #ax.set_ylim(-10, 300)
...
def sdss_spec_jpeg(DataFrame, Title, save = True, \
    ...
    if save == True and Type == '.pdf': sp_file = PdfPages(Title + Type)
    not_found = []    
    #por enquanto só as falsas, n: vermelho, p:laranja
    ...
    for i,j in enumerate(lista):
        ...
        for k,row in enumerate(j.itertuples()):
            ...
            if save == True and Type == '.jpg': sp_file = PdfPages(Title + Type)
            ...
            try:
                ...
                if save == True:
                    ...
                    plt.close('all')
                    if save == True and Type == '.jpg': sp_file.close()
    ...
    if save == True and Type == '.pdf': sp_file.close()
