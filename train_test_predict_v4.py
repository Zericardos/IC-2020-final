...
from func import prize_random, rec_all, true_falses_distribution
...
for ntreinamentos in nrange:
    ...
    feat_set_gaia, targ_set_gaia, rfc_gaia, pred_gaia, accuracy_gaia = \
        prize_random(ft_gaia, targ, prize = ntreinamentos,title = 'GAIA '+ str(ntreinamentos), save = \
                     True, return_all = 'yes')
    
    tf_gaia = true_falses_distribution(wd_table, os_table, targ_set_gaia, pred_gaia)
    ...
    ms_gaia = pd.DataFrame([tf_gaia.mean(axis = 0), tf_gaia.median(axis = 0),\
                            tf_gaia.std(axis = 0)], index = indexes)
    
    
    
    feat_splus_r = pd.read_csv('ft_splus_r_v4.csv')
    
    feat_set_splus_r, targ_set_splus_r, rfc_splus_r, pred_splus_r, accuracy_splus_r = \
        prize_random(feat_splus_r, targ, prize = ntreinamentos,title = 'S-PLUS r '+ str(ntreinamentos), save = \
                     True, return_all = 'yes')
    
    tf_splus_r = true_falses_distribution(wd_table, os_table, targ_set_splus_r, pred_splus_r)
    
    tf_splus_r.hist()
    plt.savefig('S-PLUS r ' + str(ntreinamentos) + ' histogram.pdf')
    plt.close('all')
    ms_splus_r = pd.DataFrame([tf_splus_r.mean(axis = 0), tf_splus_r.median(axis = 0),\
                               tf_splus_r.std(axis = 0)], index = indexes)
    
    feat_gaia_splus_r = pd.read_csv('ft_gaia_splus_r_v4.csv')
    
    feat_set_gaia_splus_r, targ_set_gaia_splus_r, rfc_gaia_splus_r, pred_gaia_splus_r,\
        accuracy_gaia_splus_r = prize_random(feat_gaia_splus_r, targ, prize = ntreinamentos,\
                                           title = 'GAIA S-PLUS r '+ str(ntreinamentos), save = True,\
                                               return_all = 'yes')
    
    tf_gaia_splus_r = true_falses_distribution(wd_table, os_table, \
                                             targ_set_gaia_splus_r, pred_gaia_splus_r)
    
    tf_gaia_splus_r.hist()
    plt.savefig('GAIA S-PLUS r ' + str(ntreinamentos) + ' histogram.pdf')    
    plt.close('all')
    
    ms_gaia_splus_r = pd.DataFrame([tf_gaia_splus_r.mean(axis = 0), tf_gaia_splus_r.median(axis = 0),\
                                    tf_gaia_splus_r.std(axis = 0)], index = indexes)
    
    tps = pd.DataFrame(data = (ms_gaia.tp, ms_splus_r.tp, ms_gaia_splus_r.tp),\
                       index = ['GAIA', 'S-PLUS r', 'GAIA + S-PLUS r'])
    
    tns = pd.DataFrame(data = (ms_gaia.tn, ms_splus_r.tn, ms_gaia_splus_r.tn),\
                       index = ['GAIA', 'S-PLUS r', 'GAIA + S-PLUS r'])
    
    fns = pd.DataFrame(data = (ms_gaia.fn, ms_splus_r.fn, ms_gaia_splus_r.fn),\
                       index = ['GAIA', 'S-PLUS r', 'GAIA + S-PLUS r'])
    
    fps = pd.DataFrame(data = (ms_gaia.fp, ms_splus_r.fp, ms_gaia_splus_r.fp),\
                       index = ['GAIA', 'S-PLUS r', 'GAIA + S-PLUS r'])
    ...
    xlabel = 'set'
    tfs = [tps, tns, fns, fps]
    title = ['True Positive', 'True Negative', 'False Negative', 'False Positive']
