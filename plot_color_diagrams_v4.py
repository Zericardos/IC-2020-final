...
HEADER_X = ['ujava_aper', 'f378_aper', 'f395_aper', 'f410_aper', 'f430_aper',
            ...
            'phot_rp_mean_mag']
x_gaia = ['phot_bp_mean_mag', 'phot_rp_mean_mag']
...
tfp_tfn_gaia = pd.concat([genft_df_gaia(tfp_tfn_gaia, normal=True,
                                        Index=False), tfp_tfn_gaia], axis=1)

plot_color_diagrams(tfp_tfn_gaia, 'GAIA 2', head_x=x_gaia, head_y=HEADER_Y,
                    MAG=MAGin)

tfp_tfn_splus_r = pd.read_csv('tfp_tfn_splus_r_v3.csv')
tfp_tfn_splus_r = pd.concat([genft_df_gaia(tfp_tfn_splus_r, normal=True,
                                           Index=False), tfp_tfn_splus_r],
                            axis=1)
plot_color_diagrams(tfp_tfn_splus_r, 'S-PLUS r', head_x=HEADER_X,
                    head_y=HEADER_Y, MAG=MAGin)
